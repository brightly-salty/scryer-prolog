...
impl<T: RawBlockTraits> HeapTemplate<T> {
    ...
    pub(crate) fn put_complete_string(&mut self, s: &str) -> Addr {
        ...
        if let HeapCellValue::PartialString(_, ref mut has_tail) = self[h - 1] {
            *has_tail = false;
        } else {
            unreachable!()
        }
    ...
    pub(crate) fn atom_at(&self, h: usize) -> bool {
        matches!(&self[h], HeapCellValue::Atom(..))
    }

    #[inline]
    pub(crate) fn as_unifiable(&mut self, non_heap_value: HeapCellValue) -> Addr {
    ...
    pub(crate) fn allocate_pstr(&mut self, src: &str) -> Addr {
        self.write_pstr(src).unwrap_or(Addr::EmptyList)
    }
    ...
    fn write_pstr(&mut self, mut src: &str) -> Option<Addr> {
        ...
        loop {
            ...
            let (pstr, rest_src) = PartialString::new(src);
    ...
    pub(crate) fn as_list<Iter, SrcT>(&mut self, values: Iter) -> usize
    where
    ...
    pub(crate) fn iter_from(&self, offset: usize) -> HeapIter<T> {
        HeapIter::new(&self.buf, offset * mem::size_of::<HeapCellValue>())
    }

    pub(crate) fn iter_mut_from(&mut self, offset: usize) -> HeapIterMut<T> {
    ...
    pub(crate) fn to_local_code_ptr(&self, addr: &Addr) -> Option<LocalCodePtr> {
        ...
        if let Addr::Str(s) = addr {
            if let HeapCellValue::NamedStr(arity, ref name, _) = &self[*s] {
                if let ("dir_entry", 1) = (name.as_str(), *arity) {
