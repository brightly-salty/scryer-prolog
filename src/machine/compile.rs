...
pub(super) fn compile_appendix(
    ...
    for tl in queue.iter() {
        ...
        if let Line::Control(ControlInstruction::JmpBy(_, ref mut offset, ..)) = code[jmp_by_offset]
        {
...
fn find_inner_choice_instr(code: &[Line], mut index: usize, index_loc: usize) -> usize {
    loop {
        match &code[index] {
...
fn merge_indexed_subsequences(
    ...
    if let Line::Choice(ChoiceInstruction::TryMeElse(ref mut o)) = &mut code[inner_try_me_else_loc]
    {
        ...
        match *o {
            ...
            o => match &code[inner_try_me_else_loc + o] {
                ...
                _ => {
                    code[inner_try_me_else_loc] = Line::Choice(ChoiceInstruction::RetryMeElse(o));
                }
            },
        }
    }
    ...
    if let Some(outer_threaded_clause_index) = end_of_upper_lower_bound {
        thread_choice_instr_at_to(
            code,
    ...
    } else if let Line::Choice(ChoiceInstruction::TryMeElse(ref mut o)) =
        &mut code[outer_threaded_choice_instr_loc]
    {
        retraction_info.push_record(RetractionRecord::ModifiedTryMeElse(inner_trust_me_loc, *o));
...
impl<'a> LoadState<'a> {
    fn compile(
        ...
        if settings.is_extensible {
            ...
            if let Line::Choice(ChoiceInstruction::TryMeElse(0)) = &mut code[0] {
                code_ptr += 1;
            }
    ...
    pub(super) fn incremental_compile_clause(
            ...
            .get_predicate_skeleton(&self.compilation_target, &key)
        {
            ...
            _ => {
                ...
                return self.compile(key, &[clause], &queue, settings);
            }
        };
                ...
                if mergeable_indexed_subsequences(lower_bound, target_pos, skeleton) =>
            {
                    ...
                    .switch_on_term_loc()
                {
                    Some(later_indexing_loc) if later_indexing_loc < target_indexing_loc => {
                        ...
                        if let Line::IndexingCode(indexing_code) = target_indexing_line {
                            self.retraction_info.push_record(
