...
use prolog_parser_rebis::macros::{
    alpha_char, binary_digit_char, decimal_digit_char, exponent_char, graphic_char,
...
pub fn get_key() -> KeyEvent {
    ...
    loop {
        if let Ok(Event::Key(key_)) = read() {
            if let KeyCode::Char(_) | KeyCode::Enter | KeyCode::Tab = key_.code {
                key = key_;
                break;
            }
        }
...
impl BrentAlgState {
    ...
    fn into_result(self) -> CycleSearchResult {
        match self.hare {
...
fn is_builtin_predicate(name: &ClauseName) -> bool {
    name.owning_module().as_str() == "builtins" || name.as_str().starts_with('$')
}
...
impl MachineState {
    ...
    fn repl_redirect(&mut self, repl_code_ptr: REPLCodePtr) {
    ...
    fn get_next_db_ref(&mut self, indices: &IndexStore, db_ref: &DBRef) {
        match *db_ref {
            DBRef::NamedPred(ref name, arity, _) => {
                ...
                for ((name, arity), idx) in indices.code_dir.range(key..).skip(1) {
                    if idx.is_undefined() {
                        self.fail = true;
                        return;
                    }
                    ...
                    if let Some(r) = a2.as_var() {
                        ...
                        let addr = self
                            .heap
                            .as_unifiable(HeapCellValue::DBRef(DBRef::NamedPred(
                                name.clone(),
                                *arity,
                                spec,
                            )));
            ...
            DBRef::Op(_, spec, ref name, ref op_dir, _) => {
                ...
                match op_dir.range(key..).nth(1) {
                    Some((OrderedOpDirKey(name, _), (priority, spec))) => {
                        let a2 = self[temp_v!(2)];

                        if let Some(r) = a2.as_var() {
                            let addr = self.heap.as_unifiable(HeapCellValue::DBRef(DBRef::Op(
                                *priority,
                                *spec,
                                name.clone(),
                                op_dir.clone(),
    ...
    fn fetch_attribute_goals(&mut self, mut attr_goals: Vec<Addr>) {
        ...
        let attr_goals = Addr::HeapCell(self.heap.as_list(attr_goals.into_iter()));
        ...
        match *ct {
            ...
            */
            SystemClauseType::CurrentHostname => {
                if let Ok(host) = hostname::get() {
                    if let Ok(host) = host.into_string() {
                        let hostname = self.heap.as_unifiable(HeapCellValue::Atom(
            ...
            SystemClauseType::CurrentInput => {
                ...
                match addr {
                    addr if addr.is_ref() => {
                        let stream = self.heap.as_unifiable(HeapCellValue::Stream(stream));
            ...
            SystemClauseType::DirectoryFiles => {
                let dir = self.heap_pstr_iter(self[temp_v!(1)]).as_string();
                ...
                let files_list = Addr::HeapCell(self.heap.as_list(files.into_iter()));
            ...
            SystemClauseType::FileSize => {
                ...
                let len = self.heap.as_unifiable(HeapCellValue::Integer(Rc::new(len)));
            ...
            */
            SystemClauseType::AtomChars => {
                ...
                match self.store(self.deref(a1)) {
                    Addr::Char(c) => {
                        ...
                        let list_of_chars = Addr::HeapCell(self.heap.as_list(iter));
                    ...
                    Addr::EmptyList => {
                        ...
                        let list_of_chars = Addr::HeapCell(self.heap.as_list(chars.into_iter()));

                        self.unify(a2, list_of_chars);
                    }
                    addr if addr.is_ref() => {
                        let mut iter = self.heap_pstr_iter(self[temp_v!(2)]);
                        let string = iter.as_string();
                        ...
                        match iter.focus() {
                            Addr::EmptyList => {
                                if &string == "[]" {
                                    ...
                                    let atom =
                                        self.heap.as_unifiable(HeapCellValue::Atom(chars, None));
            ...
            SystemClauseType::AtomCodes => {
                ...
                match self.store(self.deref(a1)) {
                    Addr::Char(c) => {
                        ...
                        let list_of_codes = Addr::HeapCell(self.heap.as_list(iter));
                    ...
                    Addr::EmptyList => {
                        ...
                        let list_of_codes = Addr::HeapCell(self.heap.as_list(chars.into_iter()));
                        let a2 = self[temp_v!(2)];
                    ...
                    addr if addr.is_ref() => {
                        ...
                        match self.try_from_list(temp_v!(2), stub) {
                            ...
                            Ok(addrs) => {
                                ...
                                let string = self.heap.as_unifiable(HeapCellValue::Atom(
            ...
            SystemClauseType::IsPartialString => {
                ...
                self.fail = !(matches!(
                    ...
                    Addr::AttrVar(_) | Addr::HeapCell(_) | Addr::StackCell(..) | Addr::EmptyList,
                ));
            }
            ...
            SystemClauseType::PeekChar => {
                ...
                if stream.at_end_of_stream() {
                    ...
                    let end_of_file = self
                        .heap
                        .as_unifiable(HeapCellValue::Atom(end_of_file, None));
            ...
            SystemClauseType::PeekCode => {
                ...
                let addr = match self.store(self.deref(self[temp_v!(2)])) {
                    ...
                    addr => match Number::try_from((addr, &self.heap)) {
                        Ok(Number::Integer(n)) => {
                            let n = n.to_u32().and_then(std::char::from_u32);
                        ...
                        Ok(Number::Fixnum(n)) => {
                            let n = u32::try_from(n).ok().and_then(std::char::from_u32);
            ...
            SystemClauseType::NumberToChars => {
                ...
                let chars = string.trim().chars().map(Addr::Char);
                let char_list = Addr::HeapCell(self.heap.as_list(chars));
            ...
            SystemClauseType::NumberToCodes => {
                ...
                let codes_list = Addr::HeapCell(self.heap.as_list(codes));
            ...
            */
            ...
            SystemClauseType::CharType => {
                ...
                macro_rules! associated_method_check {
                    ($id:ident, $name:tt) => {
                        if $id(c) && chars == $name {
                            self.fail = false;
                ...
                associated_method_check!(symbolic_hexadecimal_char, "symbolic_hexadecimal");
                associated_method_check!(octal_digit_char, "octal_digit");
                associated_method_check!(binary_digit_char, "binary_digit");
                associated_method_check!(hexadecimal_digit_char, "hexadecimal_digit");
                associated_method_check!(exponent_char, "exponent");
                associated_method_check!(sign_char, "sign");
                associated_method_check!(decimal_digit_char, "decimal_digit");
                associated_method_check!(symbolic_control_char, "symbolic_control");
                associated_method_check!(graphic_char, "graphic");
                associated_method_check!(graphic_token_char, "graphic_token");
                associated_method_check!(alpha_char, "alpha");
                associated_method_check!(meta_char, "meta");
                associated_method_check!(solo_char, "solo");
                associated_method_check!(prolog_char, "prolog");
                associated_method_check!(layout_char, "layout");
            ...
            SystemClauseType::PutChars => {
                ...
                let string = self.heap_pstr_iter(self[temp_v!(2)]).as_string();
            ...
            SystemClauseType::GetNChars => {
                ...
                if stream.options.stream_type == StreamType::Binary {
                    ...
                    let mut iter = self.open_parsing_stream(stream, "get_n_chars", 2)?;
            ...
            SystemClauseType::FirstStream => {
                ...
                if let Some(first_stream) = first_stream {
                    let stream = self.heap.as_unifiable(HeapCellValue::Stream(first_stream));
            ...
            SystemClauseType::NextStream => {
                ...
                for stream in indices.streams.range(prev_stream..).skip(1).cloned() {
                    if !stream.is_null_stream() {
                ...
                if let Some(next_stream) = next_stream {
                    ...
                    let next_stream = self.heap.as_unifiable(HeapCellValue::Stream(next_stream));
            ...
            */
            ...
            SystemClauseType::CopyToLiftedHeap => match self.store(self.deref(self[temp_v!(1)])) {
                Addr::Usize(lh_offset) => {
                    ...
                    for addr in self.lifted_heap.iter_mut_from(old_threshold + 1) {
                        if let HeapCellValue::Addr(ref mut addr) = addr {
            ...
            SystemClauseType::EnqueueAttributedVar => {
                ...
                if let Addr::AttrVar(h) = self.store(self.deref(addr)) {
            ...
            */
                ...
                match self.store(self.deref(a1)) {
                    Addr::Con(h) => match self.heap.clone(h) {
                        HeapCellValue::DBRef(DBRef::NamedPred(name, arity, spec)) => {
                            ...
                            let atom = self.heap.as_unifiable(HeapCellValue::Atom(name, spec));
                ...
                match self.store(self.deref(a1)) {
                    Addr::Con(h) => match self.heap.clone(h) {
                        HeapCellValue::DBRef(DBRef::Op(
                            ...
                            let a3 = self
                                .heap
                                .as_unifiable(HeapCellValue::Atom(clause_name!(spec), None));

                            let a4 = self
                                .heap
                                .as_unifiable(HeapCellValue::Atom(name, Some(shared_op_desc)));
            ...
            SystemClauseType::OpDeclaration => {
                ...
                let result = to_op_decl(priority, specifier.as_str(), op)
                    ...
                    .and_then(|mut op_decl| {
                        if op_decl.prec == 0 {
                            op_decl.remove(&mut indices.op_dir);
                            Ok(())
                        } else {
                ...
                let mut stream = match self.store(self.deref(self[temp_v!(1)])) {
                    ...
                    Addr::PStrLocation(h, n) => {
                        if let HeapCellValue::PartialString(_, true) = self.heap[h] {
                            ...
                            let file_spec = clause_name!(heap_pstr_iter.as_string(), self.atom_tbl);
            ...
            SystemClauseType::TruncateIfNoLiftedHeapGrowthDiff => {
                self.truncate_if_no_lifted_heap_diff(Addr::HeapCell)
            }
            ...
            SystemClauseType::GetAttrVarQueueBeyond => {
                ...
                if let Some(b) = b {
                    ...
                    let var_list_addr = Addr::HeapCell(self.heap.as_list(iter));
                    let list_addr = self[temp_v!(2)];
            ...
            SystemClauseType::GetLiftedHeapFromOffset => {
                ...
                if let Addr::Usize(lh_offset) = self.store(self.deref(lh_offset)) {
                    if lh_offset >= self.lifted_heap.h() {
                        let solutions = self[temp_v!(2)];
                        self.unify(solutions, Addr::EmptyList);
                    } else {
            ...
            SystemClauseType::GetDoubleQuotes => {
                ...
                match self.flags.double_quotes {
                    DoubleQuotes::Chars => {
                        let atom = self
                            .heap
                            .as_unifiable(HeapCellValue::Atom(clause_name!("chars"), None));
                    ...
                    DoubleQuotes::Atom => {
                        let atom = self
                            .heap
                            .as_unifiable(HeapCellValue::Atom(clause_name!("atom"), None));
                    ...
                    DoubleQuotes::Codes => {
                        let atom = self
                            .heap
                            .as_unifiable(HeapCellValue::Atom(clause_name!("codes"), None));
            ...
            SystemClauseType::GetSCCCleaner => {
                ...
                if let Ok(sgc_policy) = cut_policy.downcast_mut::<SCCCutPolicy>() {
            ...
            SystemClauseType::InstallInferenceCounter => {
                ...
                match a1 {
                    Addr::Usize(bp) | Addr::CutPoint(bp) => {
                        if let Ok(call_policy) = call_policy.downcast_mut::<CWILCallPolicy>() {
                            let count = call_policy.add_limit(n, bp).clone();
                            let count = self
                                .heap
                                .as_unifiable(HeapCellValue::Integer(Rc::new(count)));
            ...
            SystemClauseType::NoSuchPredicate => {
                ...
                self.fail = match self.store(self.deref(self[temp_v!(2)])) {
                    Addr::Str(s) => {
                        if let HeapCellValue::NamedStr(arity, ref name, ref spec) = self.heap[s] {
                            indices
            ...
            SystemClauseType::REPL(repl_code_ptr) => {
                self.repl_redirect(repl_code_ptr);
                return Ok(());
            }
            /*
            ...
            */
            ...
            SystemClauseType::SetInput => {
                ...
                if !stream.is_input_stream() {
                    ...
                    let user_alias = self
                        .heap
                        .as_unifiable(HeapCellValue::Atom(clause_name!("user"), None));
                ...
                match a2 {
                    Addr::CutPoint(bp) | Addr::Usize(bp) => {
                        ...
                        if prev_b <= bp {
                            let a2 = self
                                .heap
                                .as_unifiable(HeapCellValue::Atom(clause_name!("!"), None));

                            self.unify(a1, a2);
                        } else {
                            let a2 = self
                                .heap
                                .as_unifiable(HeapCellValue::Atom(clause_name!("true"), None));
            ...
            */
            ...
            SystemClauseType::ReadTermFromChars => {
                ...
                let chars = heap_pstr_iter.as_string();
            ...
            SystemClauseType::ResetContinuationMarker => {
                self[temp_v!(3)] = self
                    .heap
                    .as_unifiable(HeapCellValue::Atom(clause_name!("none"), None));
                    ...
                    match TcpListener::bind(server_addr).map_err(|e| e.kind()) {
                        Ok(tcp_listener) => {
                            ...
                            if let Some(port) = port {
                                (
                                    self.heap
                                        .as_unifiable(HeapCellValue::TcpListener(tcp_listener)),
                                    port as usize,
                                )
                            } else {
                                self.fail = true;
            ...
            SystemClauseType::SocketServerClose => match self.store(self.deref(self[temp_v!(1)])) {
                Addr::TcpListener(h) => {
            ...
            SystemClauseType::StreamProperty => {
                ...
                let property = self.heap.as_unifiable(property);
            ...
            SystemClauseType::TermAttributedVariables => {
                ...
                let outcome = Addr::HeapCell(self.heap.as_list(seen_vars.into_iter()));
            ...
            SystemClauseType::WAMInstructions => {
                ...
                let listing = Addr::HeapCell(self.heap.as_list(functor_list.into_iter()));
            ...
            SystemClauseType::ScryerPrologVersion => {
                ...
                let chars = buffer.chars().map(Addr::Char);
                let result = Addr::HeapCell(self.heap.as_list(chars));
            ...
            SystemClauseType::CryptoRandomByte => {
                ...
                let byte = self
                    .heap
                    .as_unifiable(HeapCellValue::Integer(Rc::new(Integer::from(bytes[0]))));
            ...
            SystemClauseType::CryptoCurveScalarMult => {
                ...
                let point = EcPoint::from_bytes(&group, &qbytes, &mut bnctx).unwrap();
                let scalar_bn = BigNum::from_dec_str(&scalar.to_string()).unwrap();
                let mut result = EcPoint::new(&group).unwrap();
                result.mul(&group, &point, &scalar_bn, &bnctx).ok();
            ...
            SystemClauseType::SetEnv => {
                let key = self.heap_pstr_iter(self[temp_v!(1)]).as_string();
                let value = self.heap_pstr_iter(self[temp_v!(2)]).as_string();
            ...
            SystemClauseType::CharsBase64 => {
                ...
                if self.store(self.deref(self[temp_v!(1)])).is_ref() {
                    ...
                    let mut bytes = vec![];
                    for c in self.heap_pstr_iter(self[temp_v!(1)]).as_string().chars() {
                        if c as u32 > 255 {
    ...
    pub(super) fn string_encoding_bytes(&mut self, data_arg: usize, encoding: &str) -> Vec<u8> {
        let data = self.heap_pstr_iter(self[temp_v!(data_arg)]).as_string();
    ...
    pub(super) fn xml_node_to_term(
        ...
        if node.is_text() {
            ...
            for attr in node.attributes() {
                ...
                let name = self.heap.as_unifiable(HeapCellValue::Atom(chars, None));
    ...
    pub(super) fn html_node_to_term(
        ...
        if let Some(name) = node.name() {
            ...
            self.heap.put_complete_string(&node.text())
        }
    }
}
