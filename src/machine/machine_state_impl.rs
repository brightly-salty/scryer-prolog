...
impl MachineState {
            ...
            if d1 != d2 {
                ...
                match (d1, d2) {
                    ...
                    (Addr::Str(a1), Addr::Str(a2)) => {
                        if let HeapCellValue::NamedStr(n1, ref f1, _) = self.heap[a1] {
                            if let HeapCellValue::NamedStr(n2, ref f2, _) = self.heap[a2] {
                                if n1 == n2 && *f1 == *f2 {
                                    for i in 1..n1 + 1 {
    ...
    pub(super) fn match_partial_string(&mut self, addr: Addr, string: &str, has_tail: bool) {
                ...
                match focus {
                    ...
                    addr => {
                        if has_tail {
                            ...
                            self.mode = MachineMode::Read;
                        } else if let Some(var) = addr.as_var() {
    ...
    pub(super) fn write_constant_to_var(&mut self, addr: Addr, c: &Constant) {
        match self.store(self.deref(addr)) {
                ...
                match &self.heap[c1] {
                    HeapCellValue::Atom(ref n1, _) => {
                        self.fail = match c {
                            ...
                            Constant::Char(c) if n1.is_char() => !n1.as_str().starts_with(*c),
                            _ => true,
                        };
                    }
            ...
            Addr::Char(ch) => {
                self.fail = match c {
                    Constant::Atom(ref n2, _) if n2.is_char() => !n2.as_str().starts_with(ch),
        ...
        match *instr {
            ...
            FactInstruction::GetStructure(ref ct, arity, reg) => {
                ...
                match self.store(addr) {
                    Addr::Str(a) => {
                        if let HeapCellValue::NamedStr(narity, ref s, _) = self.heap[a] {
                ...
                IndexingLine::Indexing(IndexingInstruction::Term(
                    ...
                    let offset = match addr {
                        ...
                        Addr::HeapCell(_) | Addr::StackCell(..) | Addr::AttrVar(..) => {
                            IndexingCodePtr::External(var)
                        }
                        Addr::PStrLocation(..) | Addr::Lis(_) => list,
                        Addr::Char(_)
                        | Addr::Con(_)
                        | Addr::CutPoint(_)
                        | Addr::EmptyList
    ...
    pub(crate) fn is_cyclic_term(&self, addr: Addr) -> bool {
        ...
        let is_composite =
            |addr: &Addr| matches!(*addr, Addr::Str(_) | Addr::Lis(_) | Addr::PStrLocation(..));
    ...
    pub(super) fn reset_block(&mut self, addr: Addr) {
        if let Addr::Usize(b) = self.store(addr) {
            self.block = b
        } else {
            self.fail = true
        };
    ...
    pub(super) fn execute_inlined(&mut self, inlined: &InlinedClauseType) {
        match *inlined {
            ...
            InlinedClauseType::IsCompound(r1) => {
                ...
                if let Addr::Str(_) | Addr::Lis(_) | Addr::PStrLocation(..) = d {
                    self.p += 1
                } else {
                    self.fail = true
                };
            ...
            InlinedClauseType::IsNonVar(r1) => {
                ...
                if let Addr::AttrVar(_) | Addr::HeapCell(_) | Addr::StackCell(..) = d {
                    self.fail = true;
                } else {
                    self.p += 1;
                };
            ...
            InlinedClauseType::IsVar(r1) => {
                ...
                if let Addr::AttrVar(_) | Addr::HeapCell(_) | Addr::StackCell(_, _) = d {
                    self.p += 1;
                } else {
                    self.fail = true;
                };
    ...
    fn try_functor_compound_case(
        ...
        let name = self.heap.as_unifiable(HeapCellValue::Atom(name, spec));
    ...
    fn try_functor_fabricate_struct(
        ...
        let f_a = if name.as_str() == "." && arity == 2 {
            ...
            self.heap
                .as_unifiable(HeapCellValue::NamedStr(arity, name, spec))
        };
