...
impl PartialString {
    #[inline]
    pub(super) fn new(src: &str) -> (Self, &str) {
    ...
    unsafe fn append_chars(mut self, src: &str) -> (Self, &str) {
            ...
            if let Some(r2i) = r2 {
                match (r1i, r2i) {
                    ...
                    (PStrIteratee::PStrSegment(h1, n1), PStrIteratee::PStrSegment(h2, n2)) => {
                        ...
                        ) = (&i1.machine_st.heap[h1], &i2.machine_st.heap[h2])
                        {
...
#[inline]
pub(super) fn compare_pstr_to_string(heap_pstr_iter: &mut HeapPStrIter, s: &str) -> Option<usize> {
    let mut s_offset = 0;
    ...
    while let Some(iteratee) = heap_pstr_iter.next() {
        match iteratee {
            ...
            PStrIteratee::PStrSegment(h, n) => {
                if let HeapCellValue::PartialString(ref pstr, _) = heap_pstr_iter.machine_st.heap[h]
                {
