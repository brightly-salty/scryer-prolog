...
impl fmt::Debug for StreamInstance {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Self::Bytes(ref bytes) => write!(fmt, "Bytes({:?})", bytes),
            Self::StaticStr(_) => write!(fmt, "StaticStr(_)"), // Hacky solution.
            Self::InputFile(_, ref file) => write!(fmt, "InputFile({:?})", file),
            Self::OutputFile(_, ref file, _) => write!(fmt, "OutputFile({:?})", file),
            Self::Null => write!(fmt, "Null"),
            Self::PausedPrologStream(ref put_back, ref stream) => {
                write!(fmt, "PausedPrologStream({:?}, {:?})", put_back, stream)
            }
            Self::ReadlineStream(ref readline_stream) => {
                write!(fmt, "ReadlineStream({:?})", readline_stream)
            }
            Self::Stdout => write!(fmt, "Stdout"),
            Self::TcpStream(_, ref tcp_stream) => {
                write!(fmt, "TcpStream({:?})", tcp_stream)
            }
            Self::TlsStream(_, ref tls_stream) => {
...
impl Stream {
    ...
    pub(crate) fn set_position(&mut self, position: u64) {
        if let (past_end_of_stream, StreamInstance::InputFile(_, ref mut file)) =
            self.stream_inst.0.borrow_mut().deref_mut()
        {
    ...
    pub(crate) fn is_stdout(&self) -> bool {
        matches!(self.stream_inst.0.borrow().1, StreamInstance::Stdout)
    }

    #[inline]
    pub(crate) fn is_stdin(&self) -> bool {
        matches!(
            self.stream_inst.0.borrow().1,
            StreamInstance::ReadlineStream(_)
        )
    }
    ...
    pub(crate) fn is_null_stream(&self) -> bool {
        matches!(self.stream_inst.0.borrow().1, StreamInstance::Null)
    }
    ...
    pub(crate) fn pause_stream(&mut self, buf: Vec<io::Result<char>>) -> io::Result<()> {
        if let StreamInstance::PausedPrologStream(ref mut inner_buf, _) =
    ...
    pub(crate) fn eof_action(
        ...
        match stream.options.eof_action {
            EOFAction::Error => {
                ...
                Err(self.open_past_eos_error(stream.clone(), caller, arity))
            }
            EOFAction::EOFCode => {
                let end_of_stream = if stream.options.stream_type == StreamType::Binary {
                    ...
                    self.heap
                        .as_unifiable(HeapCellValue::Atom(clause_name!("end_of_file"), None))
                };

                stream.set_past_end_of_stream();
                self.unify(result, end_of_stream);
                Ok(())
            }
            EOFAction::Reset => {
                if !stream.reset() {
                ...
                if let HeapCellValue::Atom(ref atom, ref spec) = self.heap.clone(h) {
                    match indices.stream_aliases.get(atom) {
                        ...
                        _ => {
                            ...
                            let addr = self
                                .heap
                                .as_unifiable(HeapCellValue::Atom(atom.clone(), spec.clone()));
